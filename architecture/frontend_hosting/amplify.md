...
[AWS Amplify Hosting](https://aws.amazon.com/amplify/hosting/) primarily supports static and server-side rendered web applications. It supports server-side rendering (SSR) for frameworks such as Next.js, where the App's content can be pre-rendered on the server before being delivered to the client. It also provides **DNS integration** with AWS Route 53 and SSL/TLS certificates (HTTPS). Amplify Hosting uses [Amazon CloudFront]() as its underlying CDN to automatically distribute your frontend application globally. The CDN cannot be disabled. `Amplify Hosting = S3 + CloudFront + CI/CD`.
