...
- AWS Steps functions are charge by state transitions. 
- A state transition is the movement from one state to another within a Step Functions workflow (also called a state machine). 
...
For instance, it can be used to impart structure to data stored in an Amazon S3 data lake.
