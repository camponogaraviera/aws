...
EC2 allows one to manage everything on the instance, including OS updates, scaling, load balancing, monitoring, and security. EC2 instances can also be used to manage docker containers. 
...
Use Elastic Beanstalk to simplify infrastructure management and deploy long-running containerized or non-containerized applications. 

---

# Serverless

Serverlees does not mean lack of server, it only means there is no need to manage infrastructure.
...
- Use Docker to containerize/package your Express or Apollo server.
...
As a best practice, a Lambda function should be designed to do a single task only. For example:
