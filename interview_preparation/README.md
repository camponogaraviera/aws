...
6. The Express server generates a unique S3 Object Key (e.g., `users/{user_id}/premium-images/{image_uuid}.jpg`)
...
7. The Express server uses the AWS SDK to generate a presigned S3 PutObject URL for the specific bucket, key,
and operation using the backend IAM credentials.

8. The Express server returns the URL to the client.

9. Client sends a PUT request to the <presigned-S3-URL> to upload the object directly to S3.
...
13. The Express server marks the object as COMPLETED in DynamoDB. However, if the S3 upload succeeds but the DynamoDB write fails,
...
3. The Express server retrieves the S3 object key from DynamoDB. Example: s3_key = `users/USER_id/premium-image/image_id.jpg`

4. The Express server uses the AWS SDK to generate a presigned GetObject URL.
...
3. The Express server retrieves the S3 object key from DynamoDB.

4. The Express server uses the AWS SDK to delete the data directly from S3 using the s3_key.
```
