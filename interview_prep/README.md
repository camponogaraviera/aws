...
- Use `AWS S3` to store static files (images, videos, etc.) and `DynamoDB` to store metadata (user ID, image ID, S3 object key, etc.).
...
- **Upload Flow**:
  - To get a pre-signed upload URL, the Client sends a GET/POST request to an API Gateway endpoint that invokes a Lambda function.
  - Lambda function generates a new S3 object key (`images/{hash_prefix}/{user_id}/{image_uuid}.jpg`) and a pre-signed S3 URL, and returns it to the client.
  - The Client then directly uploads the media to the S3 bucket via an HTTP PUT/POST request using that URL.

- **Download/Delete Flow**:
  - To download/delete a media from the S3 bucket, the client queries the DynamoDB table for metadata (e.g., S3 object key). 
  - Lambda function generates a pre-signed GET or DELETE URL using the S3 object key and returns it to the client.
  - The client uses the URL to fetch or delete the object directly on S3.

- **Storage Design**:
  - Images (large binary blobs) are stored in Amazon S3.
  - Metadata is stored in DynamoDB, since each DB table has a 400 KB item size limit and is optimized for key-value pairs.
  - CDN caches frequently accessed images for low-latency delivery.
